...
void IntakeOperator::updateIntakeSplit(EControllerDigital front_intake_in,
  ...
  if (run_back_intake_in) {
    m_robot->sendCommand(
        ...
        robot::subsystems::ESubsystemCommand::INTAKE_SET_BACK_VOLTAGE, 12.0);
  } else if (run_front_intake_in) {
    m_robot->sendCommand(
        robot::subsystems::ESubsystem::INTAKE,
        robot::subsystems::ESubsystemCommand::INTAKE_SET_FRONT_VOLTAGE, 12.0);
    m_robot->sendCommand(
        robot::subsystems::ESubsystem::INTAKE,
...
void IntakeOperator::update(const std::unique_ptr<profiles::IProfile>& profile) {
  EControllerDigital front_intake_in{profile->getDigitalControlMapping(EControl::INTAKE_FRONT_RUN_IN)};
  EControllerDigital front_intake_out{profile->getDigitalControlMapping(EControl::INTAKE_FRONT_RUN_OUT)};
  EControllerDigital back_intake_in{profile->getDigitalControlMapping(EControl::INTAKE_BACK_RUN_IN)};
  EControllerDigital back_arms_toggle{profile->getDigitalControlMapping(EControl::INTAKE_BACK_TOGGLE_ARMS)};

  updateIntakeSplit(front_intake_in, front_intake_out, back_intake_in, back_arms_toggle);
}
}  // namespace driftless::op_control::intake