...
void Package::addPacket(const Packet& packet) { m_packets.push_back(packet); }
...
void Package::clearPackets() { m_packets.clear(); }
...
std::vector<uint8_t> Package::getSerializedPackage() const {
  ...
  for (const Packet& packet : m_packets) {
    ...
    output_vector.insert(output_vector.end(), packet.value.begin(),
                         packet.value.end());
  }
