...
namespace driftless::control::trajectory::trajectory_generator::modifiers {
  void HolonomicSpinModifier::applyModifier(std::vector<TrajectoryPoint>& trajectory) {
    int spin_index{};
    ...
    // apply each spin, and fill space between spins with constant heading using the latest spin's end heading
    while(spin_index < m_spins.size()) {
      ...
      int accel_end_index = start_index + static_cast<int>(time_to_max_velocity / 0.02);
      int decel_start_index = end_index - static_cast<int>(time_to_max_velocity / 0.02);

      // Acceleration phase
      for (int i = start_index; i < accel_end_index && i < trajectory.size(); ++i) {
        double t = (i - start_index) * 0.02;
        trajectory[i].m_angular_velocity = direction * m_max_acceleration * t;
        trajectory[i].m_heading = initial_heading + direction * 0.5 * m_max_acceleration * t * t;
      }

      // Constant velocity phase
      for (int i = accel_end_index; i < decel_start_index && i < trajectory.size(); ++i) {
        double t = (i - accel_end_index) * 0.02;
        trajectory[i].m_angular_velocity = direction * max_angular_velocity;
        trajectory[i].m_heading = initial_heading + direction * (0.5 * m_max_acceleration * time_to_max_velocity * time_to_max_velocity + max_angular_velocity * t);
      }

      // Deceleration phase
      for (int i = decel_start_index; i < end_index && i < trajectory.size(); ++i) {
        double t = (i - decel_start_index) * 0.02;
        trajectory[i].m_angular_velocity = direction * (max_angular_velocity - m_max_acceleration * t);
        trajectory[i].m_heading = initial_heading + direction * (0.5 * m_max_acceleration * time_to_max_velocity * time_to_max_velocity + max_angular_velocity * (spin_duration - 2 * time_to_max_velocity) + max_angular_velocity * t - 0.5 * m_max_acceleration * t * t);
      }
      ...
      // Fill in the gap between the end of this spin and the start of the next spin
      int next_start_index = (spin_index + 1 < m_spins.size()) ? static_cast<int>(m_spins.at(spin_index + 1).start_t / 0.02) : trajectory.size();
      for (int i = end_index + 1; i < next_start_index && i < trajectory.size(); ++i) {
