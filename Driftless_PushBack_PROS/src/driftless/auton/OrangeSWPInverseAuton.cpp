...
OrangeSWPInverseAuton::OrangeSWPInverseAuton() : AAuton("O_SWP_T->B") {}

void OrangeSWPInverseAuton::init(
...
void OrangeSWPInverseAuton::run(
    std::shared_ptr<driftless::robot::Robot>& robot,
    std::shared_ptr<driftless::control::ControlSystem>& control_system,
  ...
  // go top goal
  goToPose(top_goal_lineup_1, MAX_VELOCITY, MAX_ANGULAR_VELOCITY / 4.0, 60.0);
  waitForGoToPose(top_goal_lineup_1, 4.0, 3000);
  goToPose(top_goal_lineup, MAX_VELOCITY, MAX_ANGULAR_VELOCITY, 60.0);
