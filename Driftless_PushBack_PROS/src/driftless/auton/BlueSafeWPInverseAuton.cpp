...
BlueSafeWPInverseAuton::BlueSafeWPInverseAuton() : AAuton("B_SWP_T->B") {}

void BlueSafeWPInverseAuton::init(
...
void BlueSafeWPInverseAuton::run(
    std::shared_ptr<driftless::robot::Robot>& robot,
    std::shared_ptr<driftless::control::ControlSystem>& control_system,
  ...
  // go middle 6 blocks
  goToPose(line_blocks_lineup, MAX_VELOCITY / 2.0, MAX_ANGULAR_VELOCITY / 5.0);
  waitForGoToPose(line_blocks_lineup, 1.5, 2500);

  goToPose(line_blocks_lineup_2, MAX_VELOCITY / 8.0,
           MAX_ANGULAR_VELOCITY / 2.0);
  waitForGoToPose(line_blocks_lineup_2, 1.0, 1000);

  goToPose(line_blocks, MAX_VELOCITY / 5.0, MAX_ANGULAR_VELOCITY / 4.0);
  ...
  goToPose(descore_2, MAX_VELOCITY, MAX_ANGULAR_VELOCITY, 20.0);
  waitForGoToPose(descore_2, 1.0, 2000);
  goToPose(descore_2_end, MAX_VELOCITY / 2.0, MAX_ANGULAR_VELOCITY, 40.0);
