...
BlueMaxPointAuton::BlueMaxPointAuton() : AAuton("B_M+10B") {}

void BlueMaxPointAuton::init(
...
void BlueMaxPointAuton::run(
  ...
  // go matchload again
  intakeStop();
  goToPose(matchload_lineup_2, MAX_VELOCITY, MAX_ANGULAR_VELOCITY, 40.0);
  ...
  goToPose(end_descore, MAX_VELOCITY / 3.0, MAX_ANGULAR_VELOCITY, 25.0);
  ...
  waitForGoToPose(top_goal_lineup, 2.0, 1750);
  hoodBumpUp();
  ...
  waitForGoToPose(top_goal_lineup, 8.0, 1750);
  hoodCloseDoor();
  goToPose(top_goal, MAX_VELOCITY / 2.0, MAX_ANGULAR_VELOCITY, 20.0);
  waitForGoToPose(top_goal, 2.0, 1000);
