...
OrangeMaxPointAuton::OrangeMaxPointAuton() : AAuton("O_M+10B") {}

void OrangeMaxPointAuton::init(
...
void OrangeMaxPointAuton::run(
  ...
  intakeStop();
  waitForGoToPose(descore_lineup, 1.0, 2000);
  goToPose(start_descore, MAX_VELOCITY / 2.0, MAX_ANGULAR_VELOCITY);
  waitForGoToPose(start_descore, 2.0, 2000);
